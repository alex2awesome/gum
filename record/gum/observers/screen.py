...
try:
    from AppKit import NSEvent, NSEventMaskKeyDown, NSEventMaskKeyUp, NSEventMaskFlagsChanged
except Exception:
    NSEvent = None
    NSEventMaskKeyDown = 0
    NSEventMaskKeyUp = 0
    NSEventMaskFlagsChanged = 0
...
class Screen(Observer):
    ...
    # Modifier state tracking for accurate key translation
    _current_modifiers: int = 0
    ...
    def __init__(
        ...
        self._last_key_position: Optional[tuple[float, float, int]] = None  # (x,y,mon_idx)
        ...
        # AppKit keyboard monitor handles
        self._ns_event_monitors: List[Any] = []
    ...
    def _update_modifier_state_from_flags(self, flags: int):
        """Update modifier state directly from NSEvent modifier flags."""
        # Subset of NSEventModifierFlag constants
        SHIFT = 0x020000
        CTRL  = 0x040000
        ALT   = 0x080000
        CMD   = 0x100000
        ...
        if flags & CMD:
            active |= CMD
        self._current_modifiers = active

    @staticmethod
    def _mon_for(x: float, y: float, mons: list[dict]) -> Optional[int]:
        # Determine which monitor contains the point (x, y)
        for idx, m in enumerate(mons, 1):
            if m["left"] <= x < m["left"] + m["width"] and m["top"] <= y < m["top"] + m["height"]:
                return idx
        return 1  # Default to first monitor if position not found
    ...
    # ─────────────────────────────── I/O helpers
    async def _save_frame(self, frame, x, y, tag: str, box_color: str = "red", box_width: int = 10, scale: float = 1.0) -> str:
        ts   = f"{time.time():.5f}"
        path = os.path.join(self.screens_dir, f"{ts}_{tag}.jpg")
        image = Image.frombytes("RGB", (frame.width, frame.height), frame.rgb)
        draw = ImageDraw.Draw(image)
        # Scale logical coordinates to pixel coordinates
    ...
    async def _worker(self) -> None:          # overrides base class
        ...
        with mss.mss() as sct:
            ...
            def schedule_key_token_event(tok: str, typ: str):
                ...
                def _register_ns_monitors():
                    ...
                    def on_down(ev):
                        ...
                        try:
                            ...
                            txt = ""
                        try:
                            self._update_modifier_state_from_flags(int(ev.modifierFlags()))
                        except Exception:
                            pass
                        tok = f"TEXT:{txt}" if txt else (f"VK:{vk}" if vk is not None else "KEY:unknown")
                        schedule_key_token_event(tok, "press")
                    ...
                    def on_up(ev):
                        ...
                        try:
                            ...
                            pass
                        tok = f"TEXT:{txt}" if txt else (f"VK:{vk}" if vk is not None else "KEY:unknown")
                        schedule_key_token_event(tok, "release")
            ...
            async def flush():
                ...
                # Compute scale factor between captured pixels and monitor logical size
                mon = mons[ev["mon"] - 1]
                try:
                ...
                bef_path = await self._save_frame(ev["before"], ev["position"][0], ev["position"][1], f"{step}_before@mon{ev['mon']}", scale=scale)
                aft_path = await self._save_frame(aft, ev["position"][0], ev["position"][1], f"{step}_after@mon{ev['mon']}", scale=scale)
            ...
            async def key_token_event(tok: str, typ: str):
                # Update modifier state is skipped here; token already computed from NSEvent
                ...
                x = x - mon["left"]
                y = y - mon["top"]
                log.info(f"Key {typ}: {tok} on monitor {idx}")

                # Log keystroke to file if configured
                ...
                async with self._key_activity_lock:
                    ...
                    # Compute scale from current frame buffer
                    try:
                        async with self._frame_lock:
                            fr = self._frames[idx]
                    ...
                    if (self._key_activity_start is None or 
                        ...
                        screenshot_path = await self._save_frame(fr, x, y, f"{step}_first", scale=scale)
                        self._key_screenshots.append(screenshot_path)
                        self._last_key_screenshot_time = current_time
                        self._last_key_position = (x, y, idx)
                        log.info(f"Started new keyboard session, saved first screenshot: {screenshot_path}")
                    else:
                        # Continue existing session - sample intermediate screenshots by time
                        should_sample = (
                            self._last_key_screenshot_time is None or
                            (current_time - self._last_key_screenshot_time) >= self._keyboard_sample_interval_sec
                        )
                        if should_sample:
                            screenshot_path = await self._save_frame(fr, x, y, f"{step}_intermediate", scale=scale)
                            self._key_screenshots.append(screenshot_path)
                            self._last_key_screenshot_time = current_time
                            self._last_key_position = (x, y, idx)
                            log.info(f"Sampled keyboard session, saved intermediate screenshot: {screenshot_path}")

                    # Schedule cleanup of previous intermediate screenshots
                    if len(self._key_screenshots) > 2:
                        asyncio.create_task(self._cleanup_key_screenshots())

            # Expose a method to accept key tokens from main thread if needed
            async def _handle_key_token_event(tok: str, typ: str):
                await key_token_event(tok, typ)
            # Bind on instance for external callers (e.g., BackgroundRecorder)
            self.handle_key_token_event = _handle_key_token_event
            ...
            async def scroll_event(x: float, y: float, dx: float, dy: float):
                ...
                async with self._frame_lock:
                    ...
                    if bf is None:
                        return
                    self._pending_event = {"type": "scroll", "position": (x, y), "mon": idx, "before": bf, "scroll": (dx, dy), "scale": scale}
            ...
            async def mouse_event(x: float, y: float, typ: str):
                ...
                async with self._frame_lock:
                    ...
                    if bf is None:
                        return
                    self._pending_event = {"type": typ, "position": (x, y), "mon": idx, "before": bf, "scale": scale}
            ...
            while self._running:                         # flag from base class
                ...
                if (self._key_activity_start is not None and 
                    ...
                    # Session ended - capture a final screenshot at last key position and cleanup
                    async with self._key_activity_lock:
                        # If we have a last key position and frame, capture a final
                        try:
                            if self._last_key_position is not None:
                                lx, ly, lidx = self._last_key_position
                                mon = mons[lidx - self._MON_START]
                                async with self._frame_lock:
                                    fr = self._frames[lidx]
                                scale = float(fr.width) / float(mon.get("width", fr.width) or fr.width) if fr else 1.0
                                final_path = await self._save_frame(fr, lx, ly, f"key_final@mon{lidx}", scale=scale)
                                self._key_screenshots.append(final_path)
                                log.info(f"Keyboard session ended, saved final screenshot: {final_path}\n")
                        except Exception:
                            pass
                        # Cleanup to keep only first and last
            ...
            if self._ns_event_monitors:
                try:
                    for m in self._ns_event_monitors:
                        try:
                            NSEvent.removeMonitor_(m)
                        except Exception:
                            pass
                finally:
                    self._ns_event_monitors = []
